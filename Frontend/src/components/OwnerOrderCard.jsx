...
import { removeOrder } from "../redux/userSlice";
...
function OwnerOrderCard({ data }) {
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/api/order/delete/${data._id}`);
    ...
    <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-6 flex flex-col gap-6 transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
      ...
      <div className="flex flex-wrap justify-between items-start gap-4">
        ...
        <div className="flex flex-col items-end gap-2">
          <div
            className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${data.payment || data.paymentMethod === "cod" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
          >
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-400 font-medium">
              OrderID: #{data._id.slice(-6).toUpperCase()}
            </span>

            {data.shopOrders.status === "delivered" && (
              <button
                onClick={handleDelete}
                className="bg-red-500 hover:bg-red-600 text-white text-xs px-2 py-1 rounded-lg font-bold"
              >
                Delete
              </button>
            )}
      ...
      <div className="bg-gray-50 rounded-2xl p-4 flex gap-3">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[10px] text-gray-400 font-bold uppercase">
            GPS: {data?.deliveryAddress.latitude.toFixed(4)},{" "}
      ...
      <div className="space-y-3">
        ...
        <div className="flex space-x-4 overflow-x-auto pb-4 no-scrollbar">
          {data.shopOrders.shopOrderItems.map((item, index) => (
            <div
              key={index}
              className="flex-shrink-0 w-44 bg-white border border-gray-100 rounded-2xl p-2 shadow-sm"
            >
              <img
                src={item.item.image}
                alt=""
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center p-4 bg-gray-50/50 rounded-2xl border border-gray-100">
          <div className="flex items-center gap-3">
            <span
              className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-tight ${getStatusColor(data.shopOrders.status)}`}
            >
        ...
        {data.shopOrders.status?.toLowerCase().replace(/\s+/g, "-") ===
          "out-for-delivery" && (
              ...
              {availableBoys?.length > 0 ? (
                availableBoys.map((b, index) => (
                  ...
                  >
                    ...
                    <span className="text-[#ff4d2d] font-medium flex items-center gap-1">
                      <MdPhone size={14} /> {b.mobile}
                    </span>
                  </div>
                ))
              ) : data.shopOrders.assignedDeliveryBoy ? (
